...
function drslon_max_icon_markup(): string {
	if ( function_exists( 'krv_max_messenger_icon_svg' ) ) {
		return krv_max_messenger_icon_svg( 'krv-landing-social-icon' );
	}

	return '<svg class="krv-landing-social-icon" viewBox="7 7 22 22" aria-hidden="true" focusable="false"><path d="M18.1,28.3c-2,0-2.9-0.3-4.4-1.5c-1,1.3-4.2,2.3-4.3,0.6c0-1.3-0.3-2.4-0.6-3.6C8.4,22.4,8,20.8,8,18.4c0-5.7,4.7-10,10.2-10S28,13,28,18.4C27.9,23.9,23.6,28.3,18.1,28.3z M18.2,13.3c-2.7-0.1-4.8,1.7-5.2,4.7c-0.4,2.4,0.3,5.4,0.9,5.5c0.3,0.1,0.9-0.5,1.4-0.9c0.7,0.5,1.5,0.8,2.4,0.9c2.8,0.1,5.2-2,5.4-4.8C23.1,15.9,20.9,13.5,18.2,13.3z"/></svg>';
}

/**
 * Header MAX link shortcode — circular chip like landing contacts under photo.
 */
function drslon_header_max_shortcode(): string {
	if ( function_exists( 'krv_max_messenger_chip_link' ) ) {
		return krv_max_messenger_chip_link(
			array(
				'aria_label' => __( 'MAX — написать в мессенджере', 'drslon-blog' ),
			)
		);
	}

	return sprintf(
		'<a class="krv-max-social-chip" href="%s" aria-label="%s" title="MAX">%s</a>',
...
function drslon_footer_connect_shortcode(): string {
	...
	$max_link = function_exists( 'krv_max_messenger_chip_link' )
		? krv_max_messenger_chip_link(
			array(
				'aria_label' => __( 'MAX — написать в мессенджере', 'drslon-blog' ),
			)
		)
	...
	return sprintf(
		'<div class="drslon-footer-connect"><p class="drslon-footer-connect__label">%s</p><div class="drslon-footer-connect__row" role="group" aria-label="%s"><a class="drslon-footer-connect__btn drslon-footer-connect__btn--telegram" href="%s" target="_blank" rel="noopener noreferrer" aria-label="%s" title="%s">%s</a><a class="drslon-footer-connect__btn drslon-footer-connect__btn--email" href="%s" aria-label="%s" title="%s">%s</a>%s</div></div>',
